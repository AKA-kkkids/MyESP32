...
void lcd_display_dir(uint8_t dir)
{
    ...
    else                                    /* 横屏 */
    {
        ...
        lcd_self.wramcmd    = 0X2C;         /* 写内存指令 */
        lcd_self.setxcmd    = 0X2A;         /* 设置X坐标指令 */
        lcd_self.setycmd    = 0X2B;         /* 设置Y坐标指令 */
    }
...
void lcd_init(void)
{
    ...
    lcd_self.dir = 0;
    lcd_self.wr = LCD_NUM_WR;                                       /* 配置WR引脚-时钟SCK */
    lcd_self.cs = LCD_NUM_CS;                                       /* 配置CS引脚-片选 */
    ...
    {
        {0x11, {0}, 0x80}, //退出睡眠模式
        {0x36, {0x00}, 1}, //设置屏幕显示方向和颜色格式
        {0x3A, {0x65}, 1}, //设置颜色格式16bit色
        {0X21, {0}, 0x80}, //开启反显功能（颜色反转）
        {0x29, {0}, 0x80}, //点亮屏幕
        {0, {0}, 0xff},
    };
